...
export default function SpecimenPage({ specimen }) {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
      <Link href="/dinoarchive/archive" style={{ color: '#1e5a8e' }}>← Back to Archive</Link>
      <h1 style={{ fontSize: '2.5rem', margin: '1rem 0' }}>{specimen.name}</h1>
      
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '2rem' }}>
        <aside style={{ background: '#f8f9fa', padding: '1.5rem', borderRadius: '8px' }}>
          <h2 style={{ fontSize: '0.9rem', color: '#666', marginBottom: '1rem' }}>Scientific Data Sheet</h2>
          ...
          {[
            ...
            ['Country', d.primaryCountry],
            ['Length', d.length],
            ['Mass', d.mass],
            ['Fossil Evidence', d.fossilEvidence],
            ['Fossil Context', d.fossilContext],
          ].map(([label, val]) => (
            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4rem 0', borderBottom: '1px solid #e0e0e0', fontSize: '0.85rem' }}>
